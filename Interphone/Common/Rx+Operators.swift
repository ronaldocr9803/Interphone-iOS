...
extension SharedSequence where Element: Sequence{
    func bindEmpty(tableView: UITableView, messageEmpty message: String = "データがありません。", textColor color: UIColor = .lightGray, immediate: Bool = false)-> Disposable{
        let isEmpty = tableView.rx.isEmpty(message: message, textColor: color)
        if immediate {
            return  self.map{ $0.count == 0 }.distinctUntilChanged().asObservable().bind(to: isEmpty)
        }
        //first time, alway init array empty eg. BehaviorSubject(value: [])
        return  self.skip(1).map{ $0.count == 0 }.distinctUntilChanged().asObservable().bind(to: isEmpty)
    }
}

extension Driver where Element: Sequence{
    func bindEmpty(collectionView: UICollectionView, messageEmpty message: String = "データがありません。", textColor color: UIColor = .black, immediate: Bool = false)-> Disposable{
        let isEmpty = collectionView.rx.isEmpty(message: message, textColor: color)
        if immediate {
            return self.map{ $0.count == 0 }.distinctUntilChanged().asObservable().bind(to: isEmpty)
        }
        return self.skip(1).map{ $0.count == 0 }.distinctUntilChanged().asObservable().bind(to: isEmpty)
